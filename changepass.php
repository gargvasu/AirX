<?php 
        $title = 'AirX Airlines | Change Password';
	...
	if(empty($_POST)===false) {
		...
		if(md5($_POST['current_password']) === $u_data['u_password']) {
...
<?php
	...
	if(empty($_POST)===false && empty($errors)===true) {
		u_change_password($session_u_id, $_POST['password']);
		header('Location: http://localhost:8383/AirX/changepass.php?success');
	}
